...
export interface DocumentWindow {
  ...
  /**
   * Grow the loaded region. `chars` rather than days (D40): a day holds between
   * ~100 KB and the 1 MB split threshold, so a day-denominated extension loads
   * an unpredictable amount and can overshoot measured-safe ground. The Pane
   * converts its screen-denominated policy into characters; the boundary lands
   * wherever `snap` puts it, which need not be a day edge.
   */
