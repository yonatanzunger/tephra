...
export abstract class SegmentedDocument implements StoredDocument {
  ...
  async #applyAndRecord(
    ...
    // (D62).
    // **`transfer` joins them**: a change of ownership is not text somebody
    // typed, so undo has no business reaching it (MH5).
    if (record && origin !== 'external' && origin !== 'boundary' && origin !== 'transfer') {
