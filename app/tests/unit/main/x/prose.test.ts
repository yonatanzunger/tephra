...
test('every prose offset survives a round trip', () => {
  for (const raw of [
    ...
    for (let i = 0; i <= p.text.length; i++) {
      assert.equal(p.map.toProse(rawAt(p.map.toDocument(proseAt(i)))), i, `prose ${i} of ${JSON.stringify(raw)}`)
    }
  }
})
...
test('a raw offset inside a marker collapses to where the marker sits', () => {
  ...
  assert.equal(p.map.toProse(rawAt(inside)), 4, 'there is nothing inside a marker to point at')
  // And every offset within the marker gives the same answer.
  const first = raw.indexOf(START('s'))
  for (let i = first; i < first + START('s').length; i++) assert.equal(p.map.toProse(rawAt(i)), 4)
})
...
test('the trailing boundary: leftmost, so typing at the end extends the range', () => {
  ...
  assert.equal(p.map.toDocument(proseAt(atEnd)), raw.indexOf(END('s')), 'inside the range, before the marker')
  assert.equal(p.map.toDocumentAfter(proseAt(atEnd)), raw.indexOf(END('s')) + END('s').length, 'and the other side is reachable')
})
...
test('a handle is one character, and the offset after it is past its bytes', () => {
  ...
  assert.equal(p.map.handleAt(proseAt(1))?.from, 1)
  assert.equal(p.map.handleAt(proseAt(0)), null)
  assert.equal(p.map.handleAt(proseAt(2)), null)
  assert.equal(p.map.toDocument(proseAt(2)), raw.length - 1, 'after the handle is after the whole comment')
})
...
test('a deletion sweeping across a range end is carved around it', () => {
  ...
  const pieces = p.map.carve(rawAt(from), rawAt(to))
...
test('a handle is NOT carved out — deleting one is a gesture with its own meaning', () => {
  ...
  const p = of(raw)
  assert.deepEqual(p.map.carve(rawAt(0), rawAt(raw.length)), [{ from: 0, to: raw.length }])
})

test('markers wholly inside a range are found, partial ones are not', () => {
  const raw = rt(`One ${START('s')}two${END('s')} three.\n`)
  const p = of(raw)
  assert.equal(p.map.within(rawAt(0), rawAt(raw.length)).length, 2)
  assert.equal(p.map.within(rawAt(0), rawAt(raw.indexOf('two'))).length, 1)
  assert.equal(p.map.within(rawAt(raw.indexOf(START('s')) + 3), rawAt(raw.length)).length, 1, 'a half-covered marker is not covered')
})
