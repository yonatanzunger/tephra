...
// what the line says.
import { isLive } from '../../src/shared/kinds/todo.ts'
...
async function settled(service: Parameters<typeof onList>[0] & {
  readonly agenda: { reconcile(): Promise<void> }
}): Promise<void> {
...
async function onList(service: {
  readonly todo: {
    ...
    items(id: DocumentId, day: DateKey): Promise<readonly { id: string | null }[]>
  }
  readonly today: DateKey
}): Promise<readonly string[]> {
  const list = await service.todo.list()
  return (await service.todo.items(list, service.today))
...
test('and renaming a matter takes its echo with it', async t => {
  // **Superseded 2026-09-18, by use.** This test asserted the opposite and
  // argued it well: *a step is something somebody wrote, and keeping it in step
  // with the matter's name would mean a rule about when they are still the same
  // thing, which is the kind of inference this design keeps refusing.*
  //
  // What falsified it: the commonest matter on a docket is one thing with one
  // step, seeded with the matter's own name (D76), and the surface draws that
  // pair as a single row. Renaming the matter alone broke the match, so
  // correcting a title grew a second line — and the thing that tells you is the
  // *rendering changing*, which is a strange way to learn about your data.
  //
  // And the feared inference turned out not to be one: the rule is an equality
  // check on exactly one step, which is a fact rather than a guess. The two
  // cases it refuses are in the D92 test below.
  const { service } = await serviced(t)
  const id = await service.library.newDocument('The house', undefined, 'docket')
  const made = await service.docket.add(id, 'Fix the skylight', { mode: 'task' })
  ...
  assert.equal(matter?.steps[0]?.text, 'Fix the roof light')
})
...
test('THE POINT OF THE PHASE: a docket puts work on the list', async t => {
  ...
  assert.deepEqual(items.map(one => one.text), ['The car needs fixing'])
...
test('THE CHAIN FIRES: finishing one brings the next', async t => {
  ...
  await service.docket.activate(id, car)
  await service.agenda.reconcile()
  await service.docket.completeStep(id, car, shop, true)
  const list = await service.todo.list()
  assert.deepEqual((await service.todo.items(list, service.today)).map(one => one.text),
...
test('a step records what it made, which is how it knows not to again', async t => {
  ...
  const step = (await service.docket.matters(id))[0]?.steps[0]
...
test('COMPLETION FLOWS BACK: finishing the TASK advances the chain', async t => {
  ...
  await service.agenda.todoSetStatus(list, first as string, 'done')
  assert.notEqual((await service.docket.matters(id))[0]?.steps[0]?.done, null,
...
test('SUSPEND WITHDRAWS what it put on the list, and only that', async t => {
  ...
  const list = await service.todo.list()
  const mine = await service.todo.add(list, 'something I typed myself')
  assert.equal((await service.todo.items(list, service.today)).length, 2)

  await service.docket.suspend(id, car)
  const left = await service.todo.items(list, service.today)
  assert.deepEqual(left.map(one => one.id), [mine], 'what a person typed is nobody else\'s business')
  assert.equal((await service.docket.matters(id))[0]?.steps[0]?.made, null)
})
...
test('and it leaves a finished one alone, because finishing it was true', async t => {
  ...
  const [made] = await onList(service)
  const list = await service.todo.list()
  await service.agenda.todoSetStatus(list, made as string, 'done')
...
test('A RECURRING TASK ADVANCES when the step its clock reads is finished', async t => {
  ...
  const list = await service.todo.list()
  await service.agenda.todoSetStatus(list, made[0] as string, 'done')
  await service.agenda.reconcile()

  const matter = (await service.docket.matters(id))[0]
...
test('THE LONG ABSENCE, calendar-driven: a year away yields ONE birthday', async t => {
  ...
  await service.docket.addStep(id, day, '7d', 'buy a present')
...
test('AN OUTSTANDING INSTANCE IS OVERDUE, not reissued (H7a)', async t => {
  ...
  await service.agenda.reconcile()
  const list = await service.todo.list()
  assert.equal((await service.todo.items(list, service.today)).length, 1, 'three days of run-up')

  await on('2026-07-01')
  await service.agenda.reconcile()
  assert.equal((await service.docket.matters(id))[0]?.when.start, '2026-03-05',
...
test('and once it is settled it catches up in one step, to the CURRENT instance', async t => {
  ...
  await on('2026-07-01')
  assert.equal((await service.docket.matters(id))[0]?.when.start, '2026-03-05', 'held, being owed')
  await service.agenda.todoSetStatus(list, made[0] as string, 'done')
  await service.agenda.reconcile()
  assert.equal((await service.docket.matters(id))[0]?.when.start, '2026-09-05',
...
test('a recurring EVENT with nothing owed rolls forward on its own', async t => {
  ...
  await on('2026-03-20')
  await service.agenda.reconcile()
  assert.equal((await service.docket.matters(id))[0]?.when.start, '2027-03-14')
})
...
test('THE CLAMP, exercised by the tick: the 31st keeps meaning the 31st', async t => {
  ...
  for (let month = 0; month < 4; month += 1) {
    const due = (await service.docket.matters(id))[0]?.when.start ?? ''
    seen.push(due)
    await on(due)
    await service.agenda.reconcile()
    const step = (await service.docket.matters(id))[0]?.steps[0]
    assert.notEqual(step?.made, null, `something to do on ${due}`)
    await service.agenda.todoSetStatus(list, step?.made as string, 'done')
    await service.agenda.reconcile()
  }
...
test('IDEMPOTENCE OVER THE WHOLE PASS: thirty runs leave one answer', async t => {
  ...
  await service.docket.add(id, 'Fix the skylight', { mode: 'task', start: '2026-03-01' })

  await on('2026-03-03')
  for (let run = 0; run < 30; run += 1) await service.agenda.reconcile()
  const list = await service.todo.list()
  const texts = (await service.todo.items(list, service.today)).map(one => one.text).sort()
...
test('AND IT WITHDRAWS, because a reconciler that only adds is an event handler', async t => {
  ...
  await service.docket.setStart(id, car, null)
  await settled(service)
  assert.equal((await service.todo.items(list, service.today)).length, 0)
  assert.equal((await service.docket.matters(id))[0]?.steps[0]?.made, null)
})
...
test('and withdrawing does not touch what a person typed, or what is done', async t => {
  ...
  await service.docket.setStart(id, car, null)
  await service.agenda.reconcile()
  assert.deepEqual((await service.todo.items(list, service.today)).map(one => one.id).sort(),
...
test('THE WEDGE: nevermind on a generated task must not silence the matter', async t => {
  ...
  await service.agenda.todoSetStatus(list, made[0] as string, 'dropped')
  const matter = (await service.docket.matters(id))[0]
...
test('and SKIPPING counts from the day it was SCHEDULED, not from today', async t => {
  ...
  await on('2026-03-24')
  await service.agenda.todoSetStatus(list, made[0] as string, 'dropped')
  assert.equal((await service.docket.matters(id))[0]?.when.start, '2026-06-10',
...
test('whereas DOING it counts from the day it was done, which is the other verb', async t => {
  ...
  await on('2026-03-24')
  await service.agenda.todoSetStatus(list, made[0] as string, 'done')
  assert.equal((await service.docket.matters(id))[0]?.when.start, '2026-06-24',
...
test('and BACKLOG resolves it too, an item nobody can see being owed by nobody', async t => {
  ...
  const list = await service.todo.list()
  const made = await onList(service)
  await service.agenda.todoSetStatus(list, made[0] as string, 'backlog')
  assert.equal((await service.docket.matters(id))[0]?.when.start, '2026-06-10')
})
...
test('THE BUG: activating a matter puts its first step on the list NOW', async t => {
  ...
  assert.notEqual((await service.docket.matters(id))[0]?.steps[0]?.made, null)
})
...
test('and every other schedule verb does the same, which is the general rule', async t => {
  ...
  await service.docket.setStart(id, talk, '2026-03-10')
  assert.equal((await service.todo.items(list, service.today)).length, 1)

  // And taking it away withdraws again, which already worked and must keep to.
  await service.docket.setStart(id, talk, null)
  assert.equal((await service.todo.items(list, service.today)).length, 0)
})
...
test('THE EVENING BUG: a step done after 4pm unblocks the next one TODAY', async t => {
  ...
  const list = await service.todo.list()
  const first = (await service.docket.matters(id))[0]?.steps[0]
  await service.agenda.todoSetStatus(list, first?.made as string, 'done')
...
test('and the horizon reads the same clock, or it would disagree with the list', async t => {
  ...
  await service.docket.activate(id, kia)
  // **Amended 2026-09-18 (D93).** The seeded step used to generate with a due
  // date, so it was the task list's row here; a generated task carries no date
  // unless something puts a clock on it, so the list contributes nothing and
  // the step two days out — still the docket's, having generated nothing — is
  // the only row. The claim survives and is what it always was: both halves
  // read the same clock, in the notebook's zone, in the hour where the zones
  // disagree.
...
test("and a name with an apostrophe is tagged too — Ada's birthday", async t => {
  ...
  const list = await service.todo.list()
  const items = await service.todo.items(list, service.today)
  assert.deepEqual(items.map(one => one.text), ["Ada's birthday"])
...
test('and the way back exists: an item says which matter made it', async t => {
  ...
  assert.deepEqual(await service.agenda.matterFor(made), { docket: id, matter: kia })

  // And a task somebody typed came from nowhere, which is not an error.
  const list = await service.todo.list()
  const mine = await service.todo.add(list, 'something I typed myself')
  assert.equal(await service.agenda.matterFor(mine), null)
})
...
test('and they arrive out of order, because nobody agrees sessions in order', async t => {
  ...
  assert.deepEqual((await service.docket.matters(id))[0]?.when.dates,
...
test('THE HORIZON READS THE LIST, rather than computing a sequence', async t => {
  ...
  const rows = await service.agenda.horizon('2026-09-01' as DateKey, '2026-10-31' as DateKey)
...
test('AND IT ADVANCES TO THE NEXT ONE WRITTEN DOWN, not to a computed date', async t => {
  ...
  await on('2026-09-21')
  await service.agenda.reconcile()
  assert.equal((await service.docket.matters(id))[0]?.when.start, '2026-10-04',
...
test('and running out of dates leaves it with NO DATE, which is the honest state', async t => {
  ...
  assert.deepEqual(await service.agenda.horizon('2026-01-01' as DateKey, '2027-01-01' as DateKey), [])
})
...
test('A GENERATED TASK CARRIES NO DUE DATE unless something is a clock', async t => {
  // **Reversed 2026-09-18 by use (D93).** This asserted the opposite, and the
  // argument was: *it arrived with no deadline, so it sorted with the undated
  // and said nothing about the rhythm it belongs to — change the water filter
  // every 120 days is not the same kind of thing as a note to self.*
  //
  // What falsified it is the commonest case rather than this one: a `+0d`
  // step's computed day is the day it generates, so nearly every task was born
  // already due and a list where everything is due today ranks nothing. The
  // cost is named where the decision is: a recurring task no longer carries its
  // rhythm onto the list, and its instance date is on the docket.
  const { service } = await serviced(t, '2026-03-10T09:00:00Z')
  const id = await service.library.newDocument('The house', undefined, 'docket')
  ...
  const list = await service.todo.list()
  const items = await service.todo.items(list, service.today)
  assert.equal(items[0]?.due, null, 'no date the app invented')
  assert.deepEqual(items.map(one => one.text), ['Change the water filter'])
})

test('but a RUN-UP step is due on the occasion it leads to', async t => {
  // **Reversed 2026-09-18 (D93), and this is the half that gained a date
  // rather than losing one.** *N days before the talk* is a genuine clock, and
  // the deadline it names is the talk: you have until the 17th, not until the
  // day you meant to start writing. The old rule said the step's own day, which
  // made a run-up overdue from the morning it appeared.
  const { service } = await serviced(t, '2026-03-10T09:00:00Z')
  const id = await service.library.newDocument('The house', undefined, 'docket')
  ...
  const list = await service.todo.list()
  const items = await service.todo.items(list, service.today)
  assert.deepEqual(items.map(one => [one.text, one.due]),
    [['write the slides', '2026-03-17']], 'the day of the talk, which is the deadline')
})
...
test('and the horizon does not show it twice, the two sources staying disjoint', async t => {
  ...
  //
  // **Amended 2026-09-18 (D93): with no invented due date, it shows in neither
  // half** — and that is the honest answer rather than a gap. The horizon is
  // *what is coming*; work that has already reached the list is here rather
  // than coming, and the list is where it will be seen. The disjointness claim
  // is untouched: what must never happen is the same commitment counted twice.
  const { service } = await serviced(t, '2026-03-10T09:00:00Z')
  const id = await service.library.newDocument('The house', undefined, 'docket')
  await service.docket.add(id, 'Change the water filter',
    { mode: 'recurring-task', every: '120d', start: '2026-03-10' })
  ...
  const rows = await service.agenda.horizon('2026-03-01' as DateKey, '2026-04-01' as DateKey)
  assert.deepEqual(rows.map(one => [one.on, one.kind, one.text]), [],
    'not twice, and not once either: it is on the list')
})
...
test('and every generated task carries its DOCKET, which is the link back', async t => {
  ...
  await service.docket.activate(work, talk)

  const list = await service.todo.list()
  assert.deepEqual(
    (await service.todo.items(list, service.today))
      .map(one => [one.text, one.tags.join('')]).sort(),
...
test("AN OWNER TRAVELS to the work the matter makes, as a marker", async t => {
  ...
  const list = await service.todo.list()
  const item = (await service.todo.items(list, service.today))[0]
  assert.equal(item?.owner, 'Sam')
  assert.equal(item.text, 'Fix the dripping tap')
})
...
test('and a matter with no owner generates a task with none, not an empty one', async t => {
  ...
  await service.docket.activate(id, tap)
  const list = await service.todo.list()
  assert.equal((await service.todo.items(list, service.today))[0]?.owner, null)
})
...
test('THE POINT: putting a task down gives it a home and leaves its line alone', async t => {
  const { service } = await serviced(t, '2026-03-10T09:00:00Z')
  const list = await service.todo.list()
  const item = await service.todo.add(list, 'repaint the shed #house OWNER Sam')

  const made = await service.agenda.todoPutDown(list, item)
  ...
  const line = (await service.todo.items(list, service.today)).find(one => one.id === item)
  assert.equal(line?.status, 'backlog')
  assert.equal(line.text, 'repaint the shed')

  // And it is a matter on the miscellaneous docket, undated and not started.
  const matters = await service.docket.matters(BACKLOG_DOCKET)
...
test('and its subjects and its owner come with it, being facts about the THING', async t => {
  const { service } = await serviced(t, '2026-03-10T09:00:00Z')
  const list = await service.todo.list()
  const item = await service.todo.add(list, 'repaint the shed #house OWNER Sam DUE 2026-03-20')
  await service.agenda.todoPutDown(list, item)

  const matter = (await service.docket.matters(BACKLOG_DOCKET))[0]
...
test('and a task a DOCKET made is not given a second home', async t => {
  ...
  assert.equal(await service.agenda.todoPutDown(list, made), null, 'nowhere new to go')
  assert.deepEqual(await service.docket.matters(BACKLOG_DOCKET), [])
})
...
test('and putting down twice does not make two of it', async t => {
  ...
  await service.agenda.todoPutDown(list, item)
  await service.agenda.todoPutDown(list, item)
  assert.equal((await service.docket.matters(BACKLOG_DOCKET)).length, 1)
})
...
test('and somewhere else, when the answer is already known', async t => {
  ...
  const { service, root } = await serviced(t, '2026-03-10T09:00:00Z')
  const house = await service.library.newDocument('The house', undefined, 'docket')
  const list = await service.todo.list()
  const item = await service.todo.add(list, 'repaint the shed')
  await service.agenda.todoPutDown(list, item, house)
  assert.deepEqual((await service.docket.matters(house)).map(one => one.name), ['repaint the shed'])
  assert.deepEqual(await service.docket.matters(BACKLOG_DOCKET), [])
})
...
test('THE BUG: putting down a CARRIED item marks the line it is on now', async t => {
  ...
  const { service, on } = await serviced(t, '2026-03-10T09:00:00Z')
  const list = await service.todo.list()
  const item = await service.todo.add(list, 'Build the checklist #peru')
  await on('2026-03-11')
  await on('2026-03-12')
  await service.todo.list()

  await service.agenda.todoPutDown(list, item)
  const now = (await service.todo.items(list, service.today)).find(one => one.id === item)
...
test('and the backlog docket lives with the others, and is NAMED', async t => {
  ...
  const { service } = await serviced(t, '2026-03-10T09:00:00Z')
  const list = await service.todo.list()
  await service.agenda.todoPutDown(list, await service.todo.add(list, 'repaint the shed'))
...
test('and the line says where it went, in the file and not only on screen', async t => {
  ...
  // **In the FILE, not only on screen** (MH5) — a day file reading `[>] repaint
  // the shed` without saying where it went is a worse record than one that says.
  // As a field now rather than a marker in the sentence (D85), so the sentence is
  // what somebody wrote and nothing else.
  await service.flush()
  const onDisk = await readFile(join(root, `${list as string}/2026/03/2026-03-10.md`), 'utf8')
  assert.match(onDisk, /moved: 'The house'/)
  assert.equal(line?.text, 'repaint the shed')
})
...
test('A MATTER MOVES BETWEEN DOCKETS, carrying what it is', async t => {
  ...
  await service.docket.addStep(BACKLOG_DOCKET, made, 'then', 'buy the paint')
  await service.docket.setNotes(BACKLOG_DOCKET, made, ['the south wall is worst'])

  const moved = await service.docket.moveTo(BACKLOG_DOCKET, made, house) as string
  assert.deepEqual(await service.docket.matters(BACKLOG_DOCKET), [], 'gone from the old one')

  const there = (await service.docket.matters(house)).find(one => one.id === moved)
...
test('and ACTIVATING is how it comes back to the list, freshly', async t => {
  ...
  await service.docket.activate(BACKLOG_DOCKET, made)
  const items = await service.todo.items(list, service.today)
  assert.deepEqual(items.filter(one => isLive(one.status)).map(one => one.text), ['repaint the shed'])
...
test('PRIMARY IS UNTOUCHABLE: a typed task is nobody else\'s business', async t => {
  ...
  await service.docket.suspend(id, car)
  const left = await service.todo.items(list, service.today)
  assert.deepEqual(left.map(one => one.text), ['Ring the dentist'])
...
test('DERIVED IS RECOVERABLE FROM PRIMARY: the step names what it made', async t => {
  ...
  await service.docket.activate(id, car)
  const list = await service.todo.list()
  const made = (await service.todo.items(list, service.today))[0]?.id ?? ''
  ...
  assert.deepEqual(await service.agenda.matterFor(made), { docket: id, matter: car })
...
test('A RESOLVED ITEM IS A FACT ABOUT THE PAST: reconcile leaves it alone', async t => {
  ...
  for (const answer of ['done', 'dropped'] as const) {
    ...
    const list = await service.todo.list()
    const made = (await service.todo.items(list, service.today))[0]?.id ?? ''
    await service.agenda.todoSetStatus(list, made, answer)
    await service.docket.suspend(id, car)
    const left = await service.todo.items(list, service.today)
...
test('AND SO DOES A TASK-LIST WRITE, because the clause reads the list', async t => {
  ...
  await service.docket.activate(id, filter)
  const list = await service.todo.list()
  const first = (await service.todo.items(list, service.today))[0]?.id ?? ''
  assert.notEqual(first, '')
  const started = (await service.docket.matters(id))[0]?.when.start

  // *Nevermind* — put down rather than done. Nobody calls reconcile.
  await service.agenda.todoSetStatus(list, first, 'dropped')

  // The instance moved on, because dropping it resolved what was outstanding.
  assert.notEqual(
    (await service.docket.matters(id))[0]?.when.start,
    started,
...
test('and a real flow stays far below the divergence threshold', async t => {
  ...
  for (const day of ['2026-04-10', '2026-05-11', '2026-06-11', '2026-07-12']) {
    const item = (await service.todo.items(list, service.today)).find(one => one.status === 'todo')
    if (item?.id != null) await service.agenda.todoSetStatus(list, item.id, 'done')
    await on(day)
  }
  assert.ok(
    service.agenda.reconciliationHighWater <= 3,
...
// ── an event that lasts, and a status step that passes (D92) ─────────

test('an event that LASTS stays on the horizon until its end, not its start', async t => {
  // **Seven of ten matters on a real events docket wrote the range into their
  // own titles** — *Santa Monica 9-17 → 9-22* — because a sweep that asked
  // about the first day dropped the trip the morning after it began. The field
  // took the arrow out of the title; this is the behaviour it bought.
  const { service, on } = await serviced(t, '2026-09-14T09:00:00Z')
  const id = await service.library.newDocument('Events', undefined, 'docket')
  const trip = await service.docket.add(id, 'Santa Monica',
    { mode: 'event', start: '2026-09-17', until: '2026-09-22' })
  const window = async (): Promise<readonly string[]> =>
    (await service.agenda.horizon(service.today, addDays(service.today, 30)))
      .filter(row => row.text.includes('Santa Monica'))
      .map(row => `${row.on}${row.until === undefined ? '' : `→${row.until}`}`)

  assert.deepEqual(await window(), ['2026-09-17→2026-09-22'], 'ahead of it: on the horizon, with both ends')

  // **The morning after it starts**, which is where this used to fall off.
  await on('2026-09-18')
  assert.deepEqual(await window(), ['2026-09-17→2026-09-22'], 'during it: still there')

  // The last day counts, and the day after does not.
  await on('2026-09-22')
  assert.deepEqual(await window(), ['2026-09-17→2026-09-22'], 'on the last day: still there')
  await on('2026-09-23')
  assert.deepEqual(await window(), [], 'after it: gone')
  void trip
})

test('a status step completes when its day is behind us, and the event is then finished', async t => {
  // **Item 3, and it deletes a special case.** D91 had to ask separately
  // whether an event's date had passed; now the status step's own completion
  // says so, and finishing is *every step done* for both kinds.
  const { service, on } = await serviced(t, '2026-09-14T09:00:00Z')
  const id = await service.library.newDocument('Events', undefined, 'docket')
  const conf = await service.docket.add(id, 'Halcyon Futures',
    { mode: 'event', start: '2026-09-18', until: '2026-09-20' })
  await service.agenda.reconcile()
  const read = async (): Promise<{ done: DateKey | null; steps: readonly boolean[] } | null> => {
    const found = (await service.docket.matters(id)).find(one => one.id === conf)
    return found === undefined ? null : {
      done: found.done,
      steps: found.steps.map(step => step.done !== null),
    }
  }
  assert.deepEqual(await read(), { done: null, steps: [false] }, 'ahead of it: nothing done')

  // **On its last day it is still on**, which is the whole reason the extent
  // has to be read here: comparing the start would have finished it on the 18th
  // while somebody was at the conference.
  await on('2026-09-20')
  assert.deepEqual(await read(), { done: null, steps: [false] }, 'on the last day: still on')

  await on('2026-09-21')
  assert.deepEqual(await read(), { done: '2026-09-21', steps: [true] }, 'the day after: complete')

  // And then it is filed, like any other finished matter (D91).
  await on('2026-09-22')
  assert.equal((await service.docket.matters(id)).length, 0)
  const archive = await service.docket.archive(id)
  assert.equal(archive?.matters, 1)
})

test('a single-day event finishes the day after it happens, not on it', async t => {
  const { service, on } = await serviced(t, '2026-09-16T09:00:00Z')
  const id = await service.library.newDocument('Events', undefined, 'docket')
  const tattoo = await service.docket.add(id, 'Owl tattoo', { mode: 'event', start: '2026-09-17' })
  await on('2026-09-17')
  assert.equal((await service.docket.matters(id)).find(one => one.id === tattoo)?.done, null,
    'on the day itself it has not passed')
  await on('2026-09-18')
  assert.equal((await service.docket.matters(id)).find(one => one.id === tattoo)?.done, '2026-09-18')
})

test('an end before the start is refused, and a recurring event carries its extent forward', async t => {
  const { service } = await serviced(t, '2026-09-14T09:00:00Z')
  const id = await service.library.newDocument('Events', undefined, 'docket')
  const trip = await service.docket.add(id, 'Lisbon', { mode: 'event', start: '2026-10-01' })
  await assert.rejects(
    () => service.docket.setUntil(id, trip, '2026-09-30'),
    /cannot end before it starts/,
  )
  // The reading form is the range somebody typed, with the arrow they used.
  await service.docket.setUntil(id, trip, '2026-10-05')
  const found = (await service.docket.matters(id)).find(one => one.id === trip)
  assert.equal(found?.when.until, '2026-10-05')
  assert.equal(readSchedule(found?.when as never, 'event'), '2026-10-01 → 2026-10-05')
})

// ── renaming a matter renames the step that echoed it ───────────

test('renaming a matter renames the one step that was its echo', async t => {
  // **Reported from use**: the commonest matter is one thing with one step,
  // seeded with the matter's own name — and renaming the matter alone broke the
  // match, so the row grew a second line. What tells you is the *rendering*
  // changing, which is a strange way to find out about your data.
  const { service } = await serviced(t)
  const id = await service.library.newDocument('Events', undefined, 'docket')
  const one = await service.docket.add(id, 'Owl Tattoo', { mode: 'event', start: service.today })
  await service.docket.rename(id, one, 'Owl tattoo, second session')
  const found = (await service.docket.matters(id)).find(m => m.id === one)
  assert.equal(found?.name, 'Owl tattoo, second session')
  assert.deepEqual(found?.steps.map(step => step.text), ['Owl tattoo, second session'])

  // **And only when it is an echo.** A step somebody has edited is their own
  // words about what has to happen, and a rename has no business rewriting it.
  const two = await service.docket.add(id, 'Fix the gate', { mode: 'task', start: service.today })
  await service.docket.editStep(id, two,
    (await service.docket.matters(id)).find(m => m.id === two)?.steps[0]?.id as string,
    'Order a new latch first')
  await service.docket.rename(id, two, 'Fix the side gate')
  const other = (await service.docket.matters(id)).find(m => m.id === two)
  assert.deepEqual(other?.steps.map(step => step.text), ['Order a new latch first'])

  // Nor when there is more than one step, however many of them match.
  const three = await service.docket.add(id, 'Paint the shed', { mode: 'task', start: service.today })
  await service.docket.addStep(id, three, '+0d', 'Buy the paint')
  await service.docket.rename(id, three, 'Paint the big shed')
  const last = (await service.docket.matters(id)).find(m => m.id === three)
  assert.deepEqual(last?.steps.map(step => step.text), ['Paint the shed', 'Buy the paint'])
})

// ── a due date only where there is a clock (D93) ─────────────

test('THE BUG: every generated task arrived already due', async t => {
  // **Reported from use.** The due date was the step's own computed day, which
  // for the commonest step there is — `+0d`, the first thing to do when work
  // starts — is the day it generates. So every task was born due today, and a
  // list where everything is due today says nothing about anything.
  const { service } = await serviced(t)
  const id = await service.library.newDocument('The house', undefined, 'docket')
  const job = await service.docket.add(id, 'Fix the gate', { mode: 'task', start: service.today })
  const list = await service.todo.list()
  const items = await service.todo.items(list, service.today)
  assert.equal(items.length, 1, 'it generated')
  assert.equal(items[0]?.due, null, 'and it has no due date')
  void job
})

test('a run-up step is due when the thing it runs up to is', async t => {
  // *N days before some event* is a real clock, and the deadline is the event:
  // you have until the conference, not until the day you meant to start.
  const { service } = await serviced(t, '2026-09-14T09:00:00Z')
  const id = await service.library.newDocument('Events', undefined, 'docket')
  const conf = await service.docket.add(id, 'The ACM talk',
    { mode: 'event', start: '2026-09-28' })
  await service.docket.addStep(id, conf, '-2w', 'Write the talk')
  await service.agenda.reconcile()
  const list = await service.todo.list()
  const items = await service.todo.items(list, service.today)
  assert.deepEqual(
    items.map(one => [one.text, one.due]),
    [['Write the talk', '2026-09-28']],
    'the run-up generated, due on the day of the talk',
  )
})

test('and an explicit DUE on the step wins, lifted out of the words', async t => {
  const { service } = await serviced(t)
  const id = await service.library.newDocument('The house', undefined, 'docket')
  const job = await service.docket.add(id, 'Fix the gate', { mode: 'task', start: service.today })
  await service.docket.addStep(id, job, '+0d', 'Order the hinge DUE 2026-03-20')
  await service.agenda.reconcile()
  const list = await service.todo.list()
  const made = (await service.todo.items(list, service.today))
    .find(one => one.text.includes('hinge'))
  assert.equal(made?.due, '2026-03-20', 'the date is in the field')
  assert.equal(made?.text, 'Order the hinge', 'and out of the sentence')
})

test('a step waiting on another step has no clock, so it has no due date', async t => {
  const { service } = await serviced(t)
  const id = await service.library.newDocument('The house', undefined, 'docket')
  const job = await service.docket.add(id, 'Fix the gate', { mode: 'task', start: service.today })
  const first = (await service.docket.matters(id)).find(m => m.id === job)?.steps[0]?.id as string
  await service.docket.addStep(id, job, 'then', 'Fit it')
  await service.docket.completeStep(id, job, first, true)
  await service.agenda.reconcile()
  const list = await service.todo.list()
  assert.deepEqual(
    (await service.todo.items(list, service.today))
      .filter(one => one.text.includes('Fit it')).map(one => one.due),
    [null],
  )
})
