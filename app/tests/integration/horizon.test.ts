...
test('AND THE TWO SOURCES DO NOT OVERLAP: a generated step is on the list, not here', async t => {
  ...
  assert.deepEqual(before.map(one => one.text), ['collect it'], 'coming, from the docket')

  // Bring its day round, and it crosses from the horizon to the list.
  ...
  assert.deepEqual(after, [], 'here now, and a generated item carries no due date')
  const list = await service.todoList()
